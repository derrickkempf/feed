...
export async function getAllProducts() {
  ...
  while (hasMore && page < 20) {
    const res = await fetch(
      `${API}/collections/all/products?storefront_token=${TOKEN}&page=${page}&size=50`
    );
...
// Create a cart, return the hosted-checkout URL (Fourthwall handles payment,
// order creation, and fulfillment from there). Falls back to the product URL.
  ...
  if (TOKEN && CHECKOUT_DOMAIN && product.variantId) {
    try {
      ...
      if (res.ok) {
        ...
        if (cart?.id) {
          return `https://${CHECKOUT_DOMAIN}/checkout/?cartCurrency=USD&cartToken=${cart.id}`;
        }
      }
    } catch {
      /* fall through */
