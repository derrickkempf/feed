...
async function sbFetchAll() {
  const [imgs, pgs] = await Promise.all([
  ...
  for (const r of imgs.data) {
    ...
    if (!byDay.has(day)) byDay.set(day, { date: day, images: [] });
  ...
  return { days: [...byDay.values()], pages };
}
...
async function lsFetchAll() {
  ...
  return {
    days: days.map((d) => ({ ...d, images: withUrls(d.images, "dl-img-") })),
